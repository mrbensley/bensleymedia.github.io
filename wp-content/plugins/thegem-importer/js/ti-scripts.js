	...
	$(function() {
		...
		var thegem_importer = {
			...
			init: function() {
				...
				$(document).on('click', '.plugin-install', function(e) {
					e.preventDefault();
					that.tryInstallPlugin($(this));
				})
			},
			...
			importClick: function(link) {
				...
				if(that.pluginCheckAjax) {
					that.pluginCheckAjax.abort();
				}
			...
			tryInstallPlugin: function($button) {
				var that = this;
				that.pluginCheckRepeater = false;
				$button.addClass('loading-hidden');
				$('<span class="loading-text">Installing...</span>').insertAfter($button);
				let plugin = $button.data('plugin');
				$.ajax({
					url: that.data.ajax_url,
					data: {action: 'thegem_importer_try_install_plugins', plugin: plugin, import: that.import},
					method: 'POST',
				}).done(function(msg) {
					that.pluginCheckRepeater = true;
					that.startPluginsCheck();
					if(msg.status == 100 && $('.thegem-importer-wrap').length) {
						const a = document.createElement("a");
						a.setAttribute('href', $button.attr('href'));
			...
			startPluginsCheck: function() {
				...
				setTimeout(function(){
					that.pluginCheckAjax = $.ajax({
						...
						if(msg.status == 200 && that.pluginCheckRepeater && $('.thegem-importer-wrap').length) {
							//that.pluginCheckRepeater = false;
							that.startPluginsCheck();
						}
					}).fail(function() {
